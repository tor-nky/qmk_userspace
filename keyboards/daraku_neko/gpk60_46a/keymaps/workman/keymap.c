/* Copyright 2021 daraku-neko / Satoru NAKAYA <@tor-nky>
 *
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_WORKMAN] = LAYOUT(
  //,-----------------------------------------------------.      ,--------------------------------------------------------------.
       OP_ESC,    KC_Q,    KC_D,    KC_R,    KC_W,    KC_B,           KC_J,    KC_F,    KC_U,    KC_P, KC_SCLN,  KC_MINS, KC_BSPC,
  //|--------+--------+--------+--------+--------+--------|      |--------+--------+--------+--------+--------+--------+--------|
      CTL_TAB,    KC_A,    KC_S,    KC_H,    KC_T,    KC_G,           KC_Y,    KC_N,    KC_E,    KC_O,    KC_I, KC_QUOT,  KC_ENT,
  //|--------+--------+--------+--------+--------+--------|      |--------+--------+--------+--------+--------+--------+--------|
...
// Ctrl, Alt, Gui のいずれかの修飾キーを押したか
#define IS_MODIFIER_KEYCODE_WITHOUT_SHIFT(code) (IS_MODIFIER_KEYCODE(code) && (code) != KC_LEFT_SHIFT && (code) != KC_RIGHT_SHIFT)
// Ctrl, Alt, Gui のいずれかの修飾キーを使った Mod-Tap キーか
#define IS_QK_MOD_TAP_NOT_ONLY_SHIFT(code) (IS_QK_MOD_TAP(code) && ((code) & ~(LSFT_T(0xFF) | RSFT_T(0xFF))))

bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  // Ctrl, Alt, Gui のいずれかの修飾キーが押されたらQWERTY
  // 参考: https://github.com/eswai/qmk_userspace/blob/27ff0dc503f39c65358e875c8d5cbeae805e01f6/users/naginata_v16/naginata_v16.c#L506
  static uint8_t n_modifier = 0;

  if (IS_MODIFIER_KEYCODE_WITHOUT_SHIFT(keycode) || IS_QK_MOD_TAP_NOT_ONLY_SHIFT(keycode)) {
