...
# 薙刀式カナ配列キーマップ
薙刀式カナ配列による入力をQMKで実現します。【薙刀式】私家版に準拠しています。
編集モードも実装していますが、
...
【薙刀式】私家版
http://oookaworks.seesaa.net/article/516730055.html#gsc.tab=0
## QMK Firmware　への組み込み方
1. [qmk_userspace/users/naginata_v16ma/](https://github.com/tor-nky/qmk_userspace/tree/main/users/naginata_v16ma) フォルダの内容を、各自の同様のフォルダを作りコピーする。
1. 各自の qmk_****/keyboards/%キーボード名%/keymaps/naginata_v16ma/ フォルダに keymap.c を作成する。
...
1. コンパイルする `qmk compile -kb %キーボード名% -km naginata_v16ma`
1. キーボードに書き込む
