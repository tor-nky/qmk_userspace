...
//  {.key = B_X|B_C|B_M    , .func = ng_send_pyu   },  // ピュ
  ...
  // 編集モード
  {.key = NG_REUSE|B_D|B_F|B_Y   , .func = ng_home}, // {Home}
  {.key = NG_REUSE|B_D|B_F|B_H   , .func = ng_edit_kakutei_end}, // {Enter}{End}
  {.key = NG_REUSE|B_D|B_F|B_N   , .func = ng_end}, // {End}
  {.key = NG_REUSE|B_D|B_F|B_U   , .func = ng_edit_delete_to_end}, // +{End}{BS}
  {.key = NG_REUSE|B_D|B_F|B_J   , .func = ng_1_up}, // {↑}
  {.key = NG_REUSE|B_D|B_F|B_M   , .func = ng_1_down}, // {↓}
  {.key = NG_REUSE|B_D|B_F|B_I   , .func = ng_saihenkan}, // {vk1Csc079}
  {.key = NG_REUSE|B_D|B_F|B_K   , .func = ng_s1_up}, // +{↑}
  {.key = NG_REUSE|B_D|B_F|B_COMM, .func = ng_s1_down}, // +{↓}
  {.key = NG_REUSE|B_D|B_F|B_O   , .func = ng_delete_with_repeat}, // {Del}
  {.key = NG_REUSE|B_D|B_F|B_L   , .func = ng_s7_up}, // +{↑ 7}
  {.key = NG_REUSE|B_D|B_F|B_DOT , .func = ng_s7_down}, // +{↓ 7}
  {.key = NG_REUSE|B_D|B_F|B_P   , .func = ng_ime_cancel}, // +{Esc 2}
  {.key = NG_REUSE|B_D|B_F|B_SCLN, .func = ng_katakana}, // ^i
  {.key = NG_REUSE|B_D|B_F|B_SLSH, .func = ng_hiragana}, // ^u
  {.key = NG_REUSE|B_J|B_K|B_Q   , .func = ng_eof}, // ^{End}
  {.key = NG_REUSE|B_J|B_K|B_A   , .func = ng_ellipsis}, // ……{改行}
  {.key = NG_REUSE|B_J|B_K|B_Z   , .func = ng_bar}, // ――{改行}
  {.key = NG_REUSE|B_J|B_K|B_W   , .func = ng_solidus}, // ／{改行}
  {.key = NG_REUSE|B_J|B_K|B_S   , .func = ng_left_white_corner_bracket}, // 『{改行}
  {.key = NG_REUSE|B_J|B_K|B_X   , .func = ng_right_white_corner_bracket}, // 』{改行}
  {.key = NG_REUSE|B_J|B_K|B_D   , .func = ng_question_mark}, // ？{改行}
  {.key = NG_REUSE|B_J|B_K|B_C   , .func = ng_exclamation_mark}, // ！{改行}
  {.key = NG_REUSE|B_J|B_K|B_R   , .func = ng_save}, // ^s
  {.key = NG_REUSE|B_J|B_K|B_F   , .func = ng_left_corner_bracket}, // 「{改行}
  {.key = NG_REUSE|B_J|B_K|B_V   , .func = ng_right_corner_bracket}, // 」{改行}
  {.key = NG_REUSE|B_J|B_K|B_T   , .func = ng_middle_dot}, // ・
  {.key = NG_REUSE|B_J|B_K|B_G   , .func = ng_left_parenthesis}, // ({改行}
  {.key = NG_REUSE|B_J|B_K|B_B   , .func = ng_right_parenthesis}, // ){改行}
  {.key = NG_REUSE|B_C|B_V|B_Y   , .func = ng_edit_s_home}, // +{Home}
  {.key = NG_REUSE|B_C|B_V|B_H   , .func = ng_copy}, // ^c
  {.key = NG_REUSE|B_C|B_V|B_N   , .func = ng_edit_s_end}, // +{End}
  {.key = NG_REUSE|B_C|B_V|B_U   , .func = ng_cut}, // ^x
  {.key = NG_REUSE|B_C|B_V|B_J   , .func = ng_1_left}, // {←}
  {.key = NG_REUSE|B_C|B_V|B_M   , .func = ng_s1_left}, // +{←}
  {.key = NG_REUSE|B_C|B_V|B_I   , .func = ng_saihenkan}, // {vk1Csc079}
  {.key = NG_REUSE|B_C|B_V|B_K   , .func = ng_1_right}, // {→}
  {.key = NG_REUSE|B_C|B_V|B_COMM, .func = ng_s1_right}, // +{→}
  {.key = NG_REUSE|B_C|B_V|B_O   , .func = ng_paste}, // ^v
  {.key = NG_REUSE|B_C|B_V|B_L   , .func = copy_spc_to_clipboard}, // {改行}{Space}+{Home}^x{BS}
  {.key = NG_REUSE|B_C|B_V|B_DOT , .func = ng_s7_left}, // +{← 7}
  {.key = NG_REUSE|B_C|B_V|B_P   , .func = ng_undo}, // ^z
  {.key = NG_REUSE|B_C|B_V|B_SCLN, .func = ng_redo}, // ^y
  {.key = NG_REUSE|B_C|B_V|B_SLSH, .func = ng_s7_right}, // +{→ 7}
  {.key = NG_REUSE|B_M|B_COMM|B_Q, .func = ng_vertical_line}, // ｜{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_A, .func = ng_left_double_angle_bracket}, // 《{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_Z, .func = ng_right_double_angle_bracket}, // 》{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_W, .func = ng_edit_separate_line}, // 　　　×　　　×　　　×{改行 2}
  {.key = NG_REUSE|B_M|B_COMM|B_S, .func = ng_left_black_lenticular_bracket}, // 【{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_X, .func = ng_right_black_lenticular_bracket}, // 】{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_E, .func = ng_edit_serifu_zengyo}, // {Home}{→}{End}{Del 2}{←}
  {.key = NG_REUSE|B_M|B_COMM|B_D, .func = ng_edit_togaki_zengyo}, // {Home}{→}{End}{Del 4}{←}
  {.key = NG_REUSE|B_M|B_COMM|B_C, .func = ng_right_corner_bracket_next_line}, // 」{改行}{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_R, .func = ng_edit_serifu}, // {Home}{改行}{Space 1}{←}
  {.key = NG_REUSE|B_M|B_COMM|B_F, .func = ng_edit_togaki}, // {Home}{改行}{Space 3}{←}
  {.key = NG_REUSE|B_M|B_COMM|B_V, .func = ng_right_corner_bracket_next_line_left_corner_bracket}, // 」{改行}{改行}「{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_T, .func = ng_white_circle}, // 〇{改行}
  {.key = NG_REUSE|B_M|B_COMM|B_G, .func = ng_edit_3_space}, // {Space 3}
  {.key = NG_REUSE|B_M|B_COMM|B_B, .func = ng_right_corner_bracket_next_line_space}, // 」{改行}{改行}{Space}
  ...
  {.key = NG_REUSE|B_V|B_M, .func = ng_enter},
...
static Ngmap_num ng_search(Ngkey searching_key) {
  ...
  for (Ngmap_num num = NGMAP_COUNT; num-- > 0; ) {  // 逆順で検索
    if (searching_key == (ngmap_key_sub(num) & ~NG_REUSE)) {
      return num;
    }
  }
  return NGMAP_COUNT;
...
static enum TransState which_trans_state(Ngkey search) {
  ...
  for (Ngmap_num num = 0; num < NGMAP_COUNT; num++) {
    Ngkey key;
    key = ngmap_key_sub(num) & ~NG_REUSE;
    // search を含む
...
static enum ReuseKeyState { Off, Ready, Run } reuse_key_state = Off;
...
bool naginata_type(uint16_t keycode, keyrecord_t *record) {
  ...
  // 出力
  {
    ...
    while (searching_count) {
      ...
      // キー再利用処理
      if (reuse_key_state == Run) {
        Ngmap_num num = ng_search(pressed_key);
#if defined(NG_REUSE_ALL) // どのかな定義もキー再利用可
        if (num < NGMAP_COUNT) {
#else // NG_REUSE を指定したかな定義のみキー再利用可
        if (num < NGMAP_COUNT && (ngmap_key_sub(num) & NG_REUSE)) {
#endif
          reuse_key_state = Off;
      ...
      if (num < NGMAP_COUNT) {
        ...
        ngmap[num].func();
#endif
#if !defined(NG_REUSE_ALL)  // NG_REUSE を指定したかな定義のみキー再利用可
        if (ngmap_key_sub(num) & NG_REUSE) {
          reuse_key_state = Ready;
        }
#endif
      }
      if (num < NGMAP_COUNT || searching_count == 1) {
