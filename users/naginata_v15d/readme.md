...
古いIMEでは使えないかもしれません。
### Mac辞書式の場合
日本語IMで変換窓が出ているときに JK+V、DF+H など、先頭にエンターがある定義が正しく動きません。
また、ライブ変換を使用中に M+Comma+Z を押すと、「　　　×　　　×　　　×」が入力できなくなります。
...
* (新MS-IME) かな変換中に Shift+英数 を押すと最初の文字が入力されない。
* (mac日本語IM) 変換窓が出ているときに JK+V、DF+H など、先頭にエンターがある定義が正しく動かない。
