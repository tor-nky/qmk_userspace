...
1. qmk_userspace/users/naginata_v15d/ フォルダの内容を、各自の同様のフォルダを作りコピーする。
1. 各自の qmk_****/keyboards/%キーボード名%/keymaps/naginata_v15d/ フォルダに keymap.c を作成する。
1. qmk_userspace/keyboards/tor_nky/coconut42/keymaps/naginata_v15d/keymap.c などを参考に、2つの `// 薙刀式` で囲まれた部分を付け加える。  
OLED を使わなければ4番目と8番目は不要です。
1. コンパイルする `qmk compile -kb %キーボード名% -km naginata_v15d`
1. キーボードに書き込む
## OSなどの設定
BLE Micro Pro の場合、フォルダ vial-qmk/keyboards/ble_micro_pro/keymaps/naginata_v15d/ と、フォルダ vial/ との違いをお調べ下さい。
### 共通
...
### Windows辞書式の場合
MS-IME以外は、キーボード設定が日本語106キーボードでなければ使えないかもしれません。

なお、ATOKはキー設定が必要です。
|キー|機能|
|---|---|
|無変換|[_文字未入力_][_記号入力_]日本語入力OFF [_他_]__入力文字種半角無変換(A)__|
